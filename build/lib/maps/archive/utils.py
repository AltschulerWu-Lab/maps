...
def conformal_prediction_sets_core(cal_df, eval_df, quantile=0.9, pred_str="Class_"):
    """Generates multiclass conformal prediction sets.
    Assumes columns {pred_str}{i} for each class i and column Label for true class.
    """
    # Find all class probability columns
    Class_cols = [col for col in cal_df.columns if re.match(rf'{pred_str}.*', col)]
    classes = [col.replace(pred_str, '') for col in Class_cols]
    
    # Compute nonconformity scores for calibration set
    def get_nonconformity(row):
        return 1 - row[f'{pred_str}{int(row["True"])}']
    ...
    def eval_pred_set(row):
        return [cl for cl in classes if 1 - row[f'{pred_str}{cl}'] <= q]
    ...
    return eval_df[['CellLines', 'Label', 'PredictionSet', 'Covered']]
